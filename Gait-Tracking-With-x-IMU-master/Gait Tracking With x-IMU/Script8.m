...
% Improved Navigation Instructions with Data Grouping
windowSize = 10;  % Number of data points to group
groupedTime = time(1:windowSize:end);
groupedPos = pos(1:windowSize:end, :);
groupedVel = vel(1:windowSize:end, :);
thresholdTurn = 100;  % 회전 감지 임계값 (도)
minStraightDuration = 8;  % 최소 직진 지속 시간 (초)
minStraightDistance = 1;  % 최소 직진 거리 (미터)
segmentDistance = 0;
lastTurnTime = groupedTime(1);
currentDirection = atan2d(groupedVel(1,2), groupedVel(1,1));
pathInstructions = {};
lastTurnDirection = 0;  % 마지막 회전 방향 초기화 (0: 직진, 1: 우회전, -1: 좌회전)

for i = 2:length(groupedTime)
    segmentDistance = segmentDistance + norm(groupedVel(i,:) * samplePeriod * windowSize);
    elapsedTime = groupedTime(i) - lastTurnTime;
    newDirection = atan2d(groupedVel(i,2), groupedVel(i,1));
    directionChange = mod(newDirection - currentDirection + 180, 360) - 180;
