...
for t = 2:length(direction)
    ...
    if directionChange >= 10 || t == length(direction)
        segmentEnd = t;
        segmentDistance = norm(pos(segmentEnd,:) - pos(segmentStart,:));
        directionChanges = [directionChanges; currentDirection, segmentDistance];
...
for i = 1:size(directionChanges, 1)
    fprintf('방향: %f도 - 거리: %.2f 미터\n', directionChanges(i,1), directionChanges(i,2));
end
...
% 이동 경로 상의 특정 이벤트 또는 점 표시 (옵션)
% plot3(pos(event_indices,1), pos(event_indices,2), pos(event_indices,3), 'bs', 'MarkerSize', 8, 'LineWidth', 3);
