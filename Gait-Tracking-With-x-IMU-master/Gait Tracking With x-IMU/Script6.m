...
% Compute the instantaneous direction of movement
direction = atan2d(vel(:,2), vel(:,1));

% Initialize variables
currentDirection = direction(1);
segmentDistance = 0;
pathInstructions = {};
thresholdTurn = 10; % 방향 변화 임계값 (도)

for t = 2:length(direction)
    % Calculate the total distance traveled in the current segment
    segmentDistance = segmentDistance + norm(vel(t,:) * samplePeriod);

    % Calculate the change in direction
    directionChange = mod(direction(t) - currentDirection + 180, 360) - 180; % -180 to 180

    % Check if the user has turned significantly
    if abs(directionChange) > thresholdTurn
        % Determine the type of turn based on the sign of direction change
        ...
        % Add the traveled distance and the turn to the navigation instructions
        if segmentDistance > 0.1 % Ignore very small segments
            pathInstructions{end+1} = sprintf('직진 %.2f 미터', segmentDistance);
            pathInstructions{end+1} = turnType;
        end

        % Reset the distance for the new segment
        segmentDistance = 0;
    end
...
% Add the final segment distance to the instructions
if segmentDistance > 0.1
    pathInstructions{end+1} = sprintf('직진 %.2f 미터', segmentDistance);
end

% Display the navigation instructions
disp('네비게이션 지침:');
